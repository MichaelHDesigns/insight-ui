...
const NodeJsServiceOptions = require('@Altcoin-Cash/dp-services-ctl/lib/services/node/NodeJsServiceOptions');
...
class InsightUIOptions extends NodeJsServiceOptions {
  ...
  mergeWithDefaultOptions(...customOptions) {
    ...
    const defaultServiceOptions = {
      config: {
        ...
        services: [
          'helpthehomelessd',
          '@Altcoin-Cash/insight-api',
          '@Altcoin-Cash/insight-ui',
          'web',
        ],
      },
    ...
    const defaultContainerOptions = {
      image: 'Altcoin-Cash/insight',
      network: {
        name: 'helpthehomeless_test_network',
        driver: 'bridge',
      },
      ports: [
        `${port}:${port}`,
      ...
      cmd: [
        '/insight/bin/helpthehomeless-node',
        'start',
      ],
      volumes: [...InsightUIOptions.defaultCustomOptions.container.volumes,
        `${configFile}:/insight/helpthehomeless-node.json`,
      ],
    };
  ...
  getConfigPath() {
    ...
    return path.join(tmpPath, 'bitcore-node-helpthehomeless.json');
  }
